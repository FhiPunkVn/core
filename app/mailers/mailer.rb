class Mailer < ActionMailer::Base
  ...
  def order_created(options)
    @person = options[:person]
    @shopping_cart = options[:person].shopping_carts.find(options[:shopping_cart_id])
    @transaction = @shopping_cart.order
    @payment_notification = @shopping_cart.payment_method.notifications.find(options[:payment_notification_id]) if options[:payment_notification_id]
  ...
  def order_failed(options)
    @person = options[:person]
    @shopping_cart = options[:person].shopping_carts.find(options[:shopping_cart_id])
    @payment_notification = @shopping_cart.payment_method.notifications.find(options[:payment_notification_id])
  ...
  def inactivity_fee_charged(options)
    @person = options[:person]
    subject = 'Bountysource Inactivity Fee Charged'
